...
namespace Vlingo.Symbio
{
    ...
    /// <typeparam name="TStateType">The type of the <typeparamref name="TState"/>.</typeparam>
    public interface IStateAdapter<TSource, TState, TStateType> where TState : State<TStateType>
    {
        /// <summary>
        ...
        TSource FromRawState(TState raw);
        ...
        TOtherState FromRawState<TOtherState>(TState raw);
        ...
        TState ToRawState(string id, TSource state, int stateVersion, Metadata metadata);
        ...
        /// <param name="metadata">The <see cref="Metadata"/> for this state</param>
        /// <returns><typeparamref name="TState"/></returns>
        TState ToRawState(TSource state, int stateVersion, Metadata metadata);
        ...
        /// <param name="stateVersion">The int state version</param>
        /// <returns><typeparamref name="TState"/></returns>
        TState ToRawState(TSource state, int stateVersion);
    }
    
    public abstract class StateAdapter<TSource, TState, TStateType> where TState : State<TStateType>
    {
        public abstract int TypeVersion { get; }

        public abstract TSource FromRawState(TState raw);

        public abstract TOtherState FromRawState<TOtherState>(TState raw);

        public virtual TState ToRawState(string id, TSource state, int stateVersion, Metadata metadata) =>
            throw new InvalidOperationException("Must override.");

        public virtual TState ToRawState(TSource state, int stateVersion, Metadata metadata) =>
            ToRawState(State<TState>.NoOp, state, stateVersion, metadata);

        public TState ToRawState(TSource state, int stateVersion) =>
