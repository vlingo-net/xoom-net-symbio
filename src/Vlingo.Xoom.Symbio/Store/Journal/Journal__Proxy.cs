...
public class Journal__Proxy<T> : IJournal<T>
{
    ...
    private const string AppendRepresentation3 =
        "Append<TSource>(string, int, TSource, Vlingo.Xoom.Symbio.Store.Journal.IAppendResultInterest, object)";

    private const string AppendRepresentation4 =
        "Append<TSource>(string, int, TSource, Vlingo.Xoom.Symbio.Metadata, Vlingo.Xoom.Symbio.Store.Journal.IAppendResultInterest, object)";

    private const string AppendWithRepresentation5 =
        "AppendWith<TSource, TSnapshotState>(string, int, TSource, TSnapshotState, Vlingo.Xoom.Symbio.Store.Journal.IAppendResultInterest, object)";

    private const string AppendWithRepresentation6 =
        "AppendWith<TSource, TSnapshotState>(string, int, TSource, Vlingo.Xoom.Symbio.Metadata, TSnapshotState, Vlingo.Xoom.Symbio.Store.Journal.IAppendResultInterest, object)";

    private const string AppendAllRepresentation7 =
        "AppendAll<TSource>(string, int, IEnumerable<TSource>, Vlingo.Xoom.Symbio.Store.Journal.IAppendResultInterest, object)";

    private const string AppendAllRepresentation8 =
        "AppendAll<TSource>(string, int, IEnumerable<TSource>, Vlingo.Xoom.Symbio.Metadata, Vlingo.Xoom.Symbio.Store.Journal.IAppendResultInterest, object)";

    private const string AppendAllWithRepresentation9 =
        "AppendAllWith<TSource, TSnapshotState>(string, int, IEnumerable<TSource>, TSnapshotState, Vlingo.Xoom.Symbio.Store.Journal.IAppendResultInterest, object)";

    private const string AppendAllWithRepresentation10 =
        "AppendAllWith<TSource, TSnapshotState>(string, int, IEnumerable<TSource>, Vlingo.Xoom.Symbio.Metadata, TSnapshotState, Vlingo.Xoom.Symbio.Store.Journal.IAppendResultInterest, object)";
    ...
    public IJournal<T> Using<TActor>(Stage stage,
        Dispatch.IDispatcher dispatcher, object[] additional)
        where TActor : Actor
    {
        if (!actor.IsStopped)
        {
    ...
    public IJournal<T> Using<TActor>(Stage stage,
        IEnumerable<Dispatch.IDispatcher> dispatchers, object[] additional)
        where TActor : Actor
    {
        if (!actor.IsStopped)
        {
